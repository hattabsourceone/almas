...
const EditAddressBilling: React.FC = () => {
  ...
  const mapBillingAddresstoPref = (item: billingAddressProps): any => {
    return {
      ...
      billing_address_2: item.address_2,
  ...
  const updateBillingAddresses = async () => {
    try {
      if (billingAddress.id === -1) return;
      if (!validateForm()) {
        return;
      }
      const body = {
        session_id,
        uid,
        ...mapBillingAddresstoPref(billingAddress),
      };
      ...
      console.log("updateBillingAddresses:", error);
    }
  };
  ...
  const getBillingAddresses = async () => {
    try {
      ...
      if (response.status === 200) {
        const list: any[] = [...response.data.billing_address];
        const result = list.find((element: any) => element.id == id);
        if (result) {
          setBillingAddress(result);
          setDefaultBillingAddress(result.is_default);
          if (result.country) {
        ...
        <Breadcrumb
          menu={[
            ...
            },
            {
              ...
              link: "/profile/address-book/edit-billing-address",
              level: 1,
            },
          ]}
        />
          ...
          <div className="w-[95%] lg:w-[80%] xl:w-[80%] 2xl:w-[72%] flex flex-col">
            ...
            <div className="flex flex-col space-y-2 pb-10">
                  ...
                  {statesList
                    ...
                    .map((e) => (
                      <option
                        key={e.name}
                        value={e.name}
                        selected={e.name === billingAddress.city}
                      >
                        {e.name}
                      </option>
                    ))}
              ...
              <div className="flex flex-row justify-between">
                ...
                <button
                  onClick={updateBillingAddresses}
                  className="text-[15px] font-medium text-white bg-[#201F41] hover:opacity-45 py-2 px-8 rounded-full"
                >
                  Continue
                </button>
